...
datos_elecciones_tidy <- datos_elecciones_tidy |>
  ...
  mutate(porc_voto = (votos / (votos_candidaturas + votos_nulos)) * 100,
         .by = c(fecha, id_mun))
```
...
orden <- datos_elecciones_tidy |> 
  filter(fecha == "2019-11") |> 
  summarise(votos = sum(votos), .by = c(fecha, siglas_comun)) |> 
  distinct(fecha, siglas_comun, .keep_all = TRUE) |> 
  arrange(desc(votos)) |> 
  pull(siglas_comun)
...
datos_elecciones_tidy |> 
  ...
  mutate(pp_psoe = ifelse(ganador == "PP", "PP", "PSOE"),
         segundo = factor(segundo, levels = orden)) |>
  ggplot() +
